...
class Federation(FederationABC):
    ...
    def from_conf(federation_session_id: str,
        ...
        port = firework_config.get("port", '9889')
    ...
    def __init__(self, session_id, party: Party,firework_config:dict):
        ...
        self._channels_map: typing.MutableMapping[_TopicKey, DataTransferServiceStub] = {}
    ...
    def get(self, name: str, tag: str, parties: typing.List[Party], gc: GarbageCollectionABC) -> typing.List:
        ...
        LOGGER.info(f"kaideng self _name_dtype_map {self._name_dtype_map}")
    ...
    def _get_channel(self, topic_pair, party_id, role, conf: dict):
        host = conf.get('host')
        port = conf.get('port')
        print(f"kaideng ==========_get_channel {host}:{port}")
    ...
    def _receive_obj(self, channel_info, name, tag):
        ...
        LOGGER.info(f"kaideng channel_info {channel_info._receive_topic}")
        ...
        while True:
            ...
            if response.code == 0:
                ...
                LOGGER.debug(f"kaideng head_str {head_str}")
    ...
    def _get_partition_receive_func(self, name, tag, party_id, role, topic_infos, session_id,conf: dict):
        def _fn(index, kvs):
            return self._partition_receive_kaideng(index, kvs, name, tag, party_id, role, topic_infos, session_id, conf)

        return _fn

    # def _partition_receive(self, index, kvs, name, tag, party_id, role, topic_infos, conf: dict):
    ...
    def _query_topic_info(self, channel_info, receive_topic, session_id):
        ...
        while needRetry & retryCount < 30:
            ...
            if response.code == 0:
                ...
                LOGGER.info(f"kaideng query {receive_topic} result {host} {port}");
                channel = grpc.insecure_channel('{}:{}'.format(host, port))
    ...
    def _handle(i):
        LOGGER.info(f"kaideng receive {i}")
    def _partition_receive_kaideng(self, index, kvs, name, tag, party_id, role, topic_infos, session_id,conf: dict):
        LOGGER.info(f"partition_receive_kaideng {index} {kvs} {name} {tag} {party_id} {role} {topic_infos} {conf}")
        topic_pair = topic_infos[index][1]
        channel_info = self._get_channel(
             topic_pair, party_id, role, conf)
        message_key_cache = set()
        ...
        while True:
            ...
            if response.code == 0:
                ...
                body = message.body
                LOGGER.info(
                    f"kaideng properties: {properties}.")
                if properties['message_id'] != name or properties['correlation_id'] != tag:
                    # leave this code to handle unexpected situation
                    # channel_info.basic_ack(message)
                    LOGGER.info(
